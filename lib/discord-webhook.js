...
// same reasoning as the unban review application above.
export async function notifyDiscordDepartmentApplication(application) {
  const embed = {
    color: 0xd0263b,
    title: 'New Department Application',
    fields: [
      { name: 'Department', value: DEPARTMENT_LABELS[application.department] || application.department, inline: true },
      { name: 'Roblox Username', value: application.robloxUsername, inline: true },
      { name: 'Discord Username', value: application.discordUsername, inline: true },
      { name: 'Age', value: application.age ? String(application.age) : 'N/A', inline: true },
      { name: 'Availability', value: application.availability || 'N/A', inline: true },
      { name: 'Prior Experience', value: application.experience || 'None listed' },
      { name: 'Why do you want to join?', value: application.reason },
    ],
  ...
  try {
    await postToDiscordChannelByName('department-applications', { embeds: [embed], components });
  } catch (error) {
