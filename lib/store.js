...
export async function getCartItems(userId) {
  const client = await db();
  const result = await client.execute({
    sql: `SELECT ci.id, ci.quantity, ci.custom_price_cents, p.id AS product_id, p.name, p.price_cents, p.is_donation
          FROM cart_items ci
...
export async function createOrderFromCart(userId) {
  ...
  try {
    await tx.execute({
      sql: 'INSERT INTO orders (id, user_id, status, total_cents) VALUES (?, ?, ?, ?)',
      args: [orderId, userId, 'pending_payment', totalCents],
    });
