...
export async function createDepartmentApplication({
  ...
  await client.execute({
    sql: `INSERT INTO department_applications
          (id, user_id, department, discord_username, email, age, experience, reason, availability, station)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [id, userId, department, discordUsername, email, age || null, experience || null, reason, availability || null, station || null],
  });
...
export async function createStaffApplication({
  ...
  const client = await db();
  const id = `staffapp_${crypto.randomUUID().slice(0, 8)}`;

  await client.execute({
    sql: `INSERT INTO staff_applications
          (id, user_id, role, discord_username, email, age, timezone, availability, prior_experience, why_join, strengths, has_microphone, scenario_1, scenario_2, scenario_3, additional_info)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      id,
      userId,
...
export async function getStaffApplication(id) {
  const client = await db();
  const result = await client.execute({ sql: 'SELECT * FROM staff_applications WHERE id = ?', args: [id] });
  return result.rows[0] || null;
}

export async function updateStaffApplicationStatus(id, status, reviewedBy) {
  const client = await db();
  await client.execute({
    sql: "UPDATE staff_applications SET status = ?, reviewed_by = ?, reviewed_at = datetime('now') WHERE id = ?",
    args: [status, reviewedBy, id],
  });
}
